...
func calculate_total_power(ship_id: String) -> int:
	...
	power += float(stats.get("crit_chance", 0)) * 2.0
	power += float(stats.get("dodge_chance", 0)) * 1.5
	power += float(stats.get("missile_speed_pct", 0)) * 0.3
...
# =============================================================================
# SKILL TREE BONUSES (Paragon / Pew Pew tree)
# =============================================================================

## Apply Paragon stat bonuses from the Pew Pew skill tree.
## These are percentage-based multipliers that stack additively.
func _apply_skill_tree_bonuses(final_stats: Dictionary) -> void:
	if not SkillManager:
		return
	
	# Stat mapping: skill stat_name → final_stats key
	var stat_keys: Array[String] = [
		"power", "fire_rate", "crit_chance", "dodge_chance",
		"max_hp", "move_speed", "missile_speed_pct", "special_cd",
		"special_damage", "damage_reduction"
	]
	
	for stat_key in stat_keys:
		var modifier: float = SkillManager.get_stat_modifier(stat_key)
		if modifier != 0.0:
			var current := float(final_stats.get(stat_key, 0))
			if stat_key == "special_cd":
				# For cooldown, negative modifier = reduction (good)
				final_stats[stat_key] = max(1.0, current * (1.0 + modifier / 100.0))
			else:
				final_stats[stat_key] = current * (1.0 + modifier / 100.0)
	
	# Re-round after bonuses
	final_stats["max_hp"] = int(round(float(final_stats.get("max_hp", 100))))
	final_stats["power"] = int(round(float(final_stats.get("power", 10))))
	final_stats["special_damage"] = int(round(float(final_stats.get("special_damage", 50))))
