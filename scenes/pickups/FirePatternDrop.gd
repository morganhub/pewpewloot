...
signal collected(pattern_id: String)
signal expired

# Strong static cache shared across all FirePatternDrop instances.
# Pattern aligned with scenes/Enemy.gd, scenes/LootDrop.gd and BonusCrystal.gd (performance_improvements.md).
...
var size_px: float = 56.0
var _time_left: float = 0.0
var _base_fall_speed: float = 220.0
...
func setup(data: Dictionary, player_ref: Node2D) -> void:
	...
	pattern_id = str(drop_data.get("pattern_id", ""))
	_player = player_ref
	despawn_time_sec = maxf(0.1, float(drop_data.get("despawn_time_sec", 8.0)))
	pickup_radius = maxf(8.0, float(drop_data.get("pickup_radius", 28.0)))
	magnet_speed = maxf(10.0, float(drop_data.get("magnet_speed", 420.0)))
	size_px = maxf(8.0, float(drop_data.get("size_px", 56.0)))
	_base_fall_speed = maxf(40.0, float(drop_data.get("fall_speed_px_sec", 220.0)))
...
func _apply_visual() -> void:
	...
	var asset_path: String = str(drop_data.get("asset", "")).strip_edges()
